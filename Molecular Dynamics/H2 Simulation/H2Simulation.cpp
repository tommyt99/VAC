...
const int tmax = 50;
const int n = 4; //This has to be an even number. 
...
double ecut;
double rc = box / 2;
const double sigma = 1.06; //[A] Angstrom
const double k = 100;//1.1*pow(10,-7); //[E/sigma^2]
const double r0 = 0.751; //[A]Angstrom
...
void force() {
	en = 0;
	double xr, yr, zr, r2, r2i, r6i, ff, fb_x, fb_y, fb_z, R;
	for (int i = 0; i < npart; i++) {
		f[i][0] = 0;
		f[i][1] = 0;
		f[i][2] = 0;
	...
	for (int i = 0; i < npart - 1; i++) {
		//Harmonic potential (bond between hydrogen atoms) 
		R = sqrt((pow(x[i + 1][0], 2) - (2 * x[i + 1][0] * x[i][0]) + pow(x[i][0], 2)) + (pow(x[i + 1][1], 2) - (2 * x[i + 1][1] * x[i][1]) + pow(x[i][1], 2)) + (pow(x[i + 1][2], 2) - (2 * x[i + 1][2] * x[i][2]) + pow(x[i][2], 2)));
		fb_x = 0.5*k*(2 * x[i + 1][0] - 2 * x[i][0]) - ((r0*(2 * x[i + 1][0] - 2 * x[i][0])) / R);
		fb_y = 0.5*k*(2 * x[i + 1][1] - 2 * x[i][1]) - ((r0*(2 * x[i + 1][1] - 2 * x[i][1])) / R);
		fb_z = 0.5*k*(2 * x[i + 1][2] - 2 * x[i][2]) - ((r0*(2 * x[i + 1][2] - 2 * x[i][2])) / R);
		if (i % 2 == 0) {
			f[i][0] += fb_x;
			f[i + 1][0] -= fb_x;
	...
	for (int i = 0; i < npart - 1; i++) {
		for (int j = i + 1; j < npart; j++) {
			...
			if (r2 < rc*rc) { //Periodic Boundary Condition
				r2i = 1 / r2;
				r6i = pow(r2i, 3);
				ff = 48 * r2i*r6i*(r6i - 0.5); // Lennard-Jones
...
int main() {
	...
	ofstream coordinates("coordinates.xyz");
	coordinates << npart << endl;
	...
	while (t < tmax) {
		...
		if (timestep == 10) { // data of xyz coordinates for all particles every 1000 steps. 
			...
			for (int i = 0; i < npart; i++) {
				coordinates << "ATOM" << i << " " << x[i][0] << "  " << x[i][1] << "  " << x[i][2] << endl;
			}
		}
